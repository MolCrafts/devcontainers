...
Both are `mcr.microsoft.com/devcontainers/base:ubuntu-24.04` plus every toolchain feature. `molcrafts-cuda` adds `nvidia-cuda` (nvcc, cuDNN, NVTX) and declares `hostRequirements.gpu: "optional"`, so it still opens on a machine without a GPU.
