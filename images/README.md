...
| Image | Platforms | Base | Adds over the other |
|---|---|---|---|
| `ghcr.io/molcrafts/devcontainers/images/molcrafts-cpu` | `linux/amd64`, `linux/arm64` | Ubuntu 24.04 | — |
| `ghcr.io/molcrafts/devcontainers/images/molcrafts-cuda` | `linux/amd64` | **`nvidia/cuda:*-cudnn-devel-ubuntu24.04`** | nvcc · toolkit · cuDNN from NVIDIA |
...
**CPU base:** `mcr.microsoft.com/devcontainers/base:ubuntu-24.04`  
**CUDA base:** `nvidia/cuda:12.8.1-cudnn-devel-ubuntu24.04` (official NVIDIA devel + cuDNN — not the broken `devcontainers/features/nvidia-cuda` package install on bare Ubuntu)
...
We do **not** use `nvcr.io/nvidia/pytorch` as the base: that image pins Python to whatever NGC ships (behind the 3.14 the ecosystem targets) and costs ~10 GB before a single MolCrafts tool is installed. Worse, a torch in the base image is a *different* environment from the project's `.venv`, and molnex links against torch at build time via `find_package(Torch)`. Two torches on one machine is an ABI mismatch waiting to happen.

The CUDA image uses NVIDIA's **CUDA toolkit** image only (`nvidia/cuda:…-cudnn-devel-…`) so `nvcc` / cuDNN come from the vendor, while torch still comes from `uv sync`.
