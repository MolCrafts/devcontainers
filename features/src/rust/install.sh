...
hand_over_to_remote_user() {
    ...
    if ! id -u "${USERNAME}" >/dev/null 2>&1; then
        log "WARN: remote user ${USERNAME} not found; leaving CARGO_HOME owned by root"
        return 0
    fi
    # Without this the remote user cannot `cargo install`, and `cargo build`
    # cannot write the registry cache under CARGO_HOME.
    log "chown ${CARGO_HOME} ${RUSTUP_HOME} -> ${USERNAME}"
    chown -R "${USERNAME}:$(id -gn "${USERNAME}")" "${CARGO_HOME}" "${RUSTUP_HOME}"
}
