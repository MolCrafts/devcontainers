...
# build. Wire PATH via profile.d + symlinks into /usr/local/bin instead.
expose_on_path() {
...
EOF
    ...
    # Non-login shells (feature tests, CI scripts) often skip profile.d. Mirror
    # every tool and interpreter symlink into /usr/local/bin, which is always on
    # the default PATH.
...
hand_over_to_remote_user() {
    ...
    if ! id -u "${USERNAME}" >/dev/null 2>&1; then
        log "WARN: remote user ${USERNAME} not found; leaving ${UV_ROOT} owned by root"
        return 0
    fi

    # The remote user must be able to `uv tool install` more tools and let uv
    # write interpreter metadata, otherwise every post-create step needs sudo.
    log "chown ${UV_ROOT} -> ${USERNAME}"
    chown -R "${USERNAME}:$(id -gn "${USERNAME}")" "${UV_ROOT}"
}
